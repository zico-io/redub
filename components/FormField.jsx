...
const FormField = ({ title, value, placeholder, handleChangeText, otherStyles, ...props }) => {
  ...
  return (
    <View className={`space-y-2 ${otherStyles}`}>
      <Text className='text-base text-white font-sfmedium'>{title}</Text>
      ...
      <View className='border-2 border-black-200 w-full h-16 px-4 bg-black-200 rounded-2xl focus:border-action items-center flex-row'>
        ...
        {title === 'Password' && (<TouchableOpacity onPress={() => setShowPassword(!showPassword)}>
            <Image source={!showPassword ? icons.eye : icons.eyehide } tintColor='#CCC5B9' className='w-6 h-6' resizeMode='contain'/>
        </TouchableOpacity>)}
      </View>
    </View>
  )
